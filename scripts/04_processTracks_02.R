...
#--------------------------------------------------------------------------
## Prefilter
# dat <- foieGras::fit_ssm(d = dat, pf = TRUE) # This is failing with a strange error
...
if(TRUE) {
  ...
  for (i in ids) {
    ...
    tryCatch({
      ## Fit SSM
      this_fit <- foieGras::fit_ssm(this_dat, pf = FALSE, spdf = FALSE, model = "rw", time.step = hrs)
      ## Grab the output
      this_out <- grab(this_fit, "predicted", as_sf = FALSE)
      ## Store the aic
      this_aic <- this_fit$ssm[[1]]$aic
  ...
  if (FALSE) {
    ...
    pdf("./out/checkTracksFoieGras.pdf", paper = "a4", useDingbats = F)
    for (i in ids) {
      print(i)
      this.d <- out[out$id == i, ]
...
# Save
saveRDS(out, paste0("./out/foieGras_ssm_", hrs, ".RDS"))
saveRDS(out, paste0("Y:/PROJECTS/reisinger_humpbacks/mega/out/foieGras_ssm_", hrs, ".RDS"))
...
this_mpm_out <- cbind(mpm_out_dat, mpm_out_fit[ , c("g", "g.se")])
...
saveRDS(mpm_out, paste0("./out/foieGras_mpm_", hrs, ".RDS"))
saveRDS(mpm_out, paste0("Y:/PROJECTS/reisinger_humpbacks/mega/out/foieGras_mpm_", hrs, ".RDS"))
